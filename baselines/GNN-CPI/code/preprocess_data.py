...
if __name__ == "__main__":
    ...
    data_path = os.path.join(PROJECT_ROOT, 'dataset', DATASET, 'original', 'data.txt')
    ...
    with open(os.path.join(dir_input, 'Smiles.txt'), 'w') as f:
        f.write(Smiles)
    np.save(os.path.join(dir_input, 'compounds'), compounds)
    np.save(os.path.join(dir_input, 'adjacencies'), adjacencies)
    np.save(os.path.join(dir_input, 'proteins'), proteins)
    np.save(os.path.join(dir_input, 'interactions'), interactions)
