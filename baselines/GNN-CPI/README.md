...
(i) Create the tensor data of CPIs with the following command on Windows:
```
cd code
preprocess_data.bat
```
...
(ii) Using the preprocessed data, train the model with the following command on Windows:
```
run_training.bat
```
...
(iii) You can change the hyperparameters in preprocess_data.bat and run_training.bat.
