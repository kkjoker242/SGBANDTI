...
import math
import numpy as np
from sklearn.metrics import roc_auc_score, precision_score, recall_score, f1_score, confusion_matrix, precision_recall_curve, auc
...
class Tester(object):
    ...
    def test(self, dataset, full_metrics=False):
        self.model.eval()
        N = len(dataset)
        T, Y, S = [], [], [] # True labels, Predicted labels, Scores
        with torch.no_grad():
            for data in dataset:
        ...
        if not full_metrics:
            # Original behavior for validation set
            return auroc, auprc, f1_score(T, Y)

        # Calculate full metrics for the test set
        f1 = f1_score(T, Y)
        ...
        return auroc, auprc, f1, sensitivity, specificity, accuracy

    def get_predictions(self, dataset):
        """获取测试集的预测分数和真实标签，用于绘制ROC和PRC曲线"""
        self.model.eval()
        T, S = [], [] # True labels, Scores
        with torch.no_grad():
            for data in dataset:
                correct_labels, predicted_labels, predicted_scores = self.model(data, train=False)
                T.append(correct_labels)
