...
import os
import numpy as np
from sklearn.metrics import roc_auc_score, average_precision_score, roc_curve, accuracy_score, precision_recall_curve, precision_score, recall_score, matthews_corrcoef, confusion_matrix
...
class Trainer(object):
    ...
    def train(self):
        ...
        auroc, auprc, f1, sensitivity, specificity, accuracy, test_loss, thred_optim = self.test(dataloader="test")
        ...
        print('Test at Best Model of Epoch ' + str(self.best_epoch) + ' with test loss ' + str(test_loss), " AUROC "
              + str(auroc) + " AUPRC " + str(auprc) + " F1 " + str(f1) + " Sensitivity " + str(sensitivity) +
    ...
    def test(self, dataloader="test"):
        ...
        if dataloader == "test":
            ...
            np.save(os.path.join(self.output_dir, "roc_fpr.npy"), fpr)
            np.save(os.path.join(self.output_dir, "roc_tpr.npy"), tpr)
            np.save(os.path.join(self.output_dir, "pr_precision.npy"), prec)
            np.save(os.path.join(self.output_dir, "pr_recall.npy"), recall)
            np.save(os.path.join(self.output_dir, "y_true.npy"), np.array(y_label))
            np.save(os.path.join(self.output_dir, "y_score.npy"), np.array(y_pred))
            try:
            ...
            cm = confusion_matrix(y_label, y_pred_s, labels=[0, 1])
            tn, fp, fn, tp = cm.ravel()
            sensitivity = tp / (tp + fn + 1e-12)
            specificity = tn / (tn + fp + 1e-12)

            return auroc, auprc, np.max(f1[5:]), sensitivity, specificity, accuracy, test_loss, thred_optim
        else:
