...
# ESPF encoding (lazy loading to avoid import errors when ESPF files are missing)
dbpe = None
pbpe = None
idx2word_d = None
words2idx_d = None
...
def _load_espf():
    global dbpe, pbpe, idx2word_d, words2idx_d, idx2word_p, words2idx_p
    if dbpe is not None:
        return
...
def draw_roc_curve(y_pred, y_label, figure_file, method_name):
	'''
...
def create_fold(df, fold_seed, frac):
    ...
    #test.to_csv('/home/storage2/yan/code/Multi_Scale_DTI/DeepPurpose/dataset/test.csv')
    #train.to_csv('/home/storage2/yan/code/Multi_Scale_DTI/DeepPurpose/dataset/train.csv')
    #val.to_csv('/home/storage2/yan/code/Multi_Scale_DTI/DeepPurpose/dataset/val.csv')

    '''
    #load data--------------------------------------------
    test_load = pd.read_csv('/home/yan/code/Multi_Scale_DTI/DeepPurpose/dataset/test.csv')
    train_load = pd.read_csv('/home/yan/code/Multi_Scale_DTI/DeepPurpose/dataset/train.csv')
    val_load = pd.read_csv('/home/yan/code/Multi_Scale_DTI/DeepPurpose/dataset/val.csv')
    test_index = test_load.iloc[:,0].values
    train_index = train_load.iloc[:,0].values
    val_index = val_load.iloc[:,0].values
...
def encode_drug(df_data, drug_encoding, column_name = 'SMILES', save_column_name = 'drug_encoding'):
	...
	if drug_encoding == 'Morgan':
		...
		unique_dict = dict(zip(df_data[column_name].unique(), unique))
		df_data[save_column_name] = [unique_dict[i] for i in df_data[column_name]]
	elif drug_encoding == 'CNN' or drug_encoding == 'Inception' or drug_encoding=='CNN1D':
...
def encode_protein(df_data, target_encoding, column_name = 'Target Sequence', save_column_name = 'target_encoding'):
	...
	if target_encoding == 'AAC':
		...
		AA_dict = dict(zip(df_data[column_name].unique(), AA))
		df_data[save_column_name] = [AA_dict[i] for i in df_data[column_name]]
	elif target_encoding == 'CNN' or target_encoding == 'Inception' or target_encoding == 'CNN1D':
...
def drug_to_hilbert(v,H):
    v = v.T
    d_1,d_2 = H.shape
    DATA_D = -1. * np.ones((d_1, d_2, v.shape[1]))
    DATA_D = plot_hb_dna(seq=v, H_curve=H)
    return DATA_D

def protein_to_hilbert(v,H):
    v = v.T
    d_1,d_2 = H.shape
    DATA_P = -1. * np.ones((d_1, d_2, v.shape[1]))
    DATA_P = plot_hb_dna(seq=v, H_curve=H)
    return DATA_P
...
class BIN_Data_Encoder(data.Dataset):
    ...
    def __init__(self, list_IDs, labels, df_dti, feature_p, phase, **config):
        'Initialization'
        self.labels = labels
        ...
        order_D = 16
        self.H_D = hilbert_curve(order_D)[:8,:]

        '''
        self.pos_D = np.zeros((100,2)).astype(int)
        for i in range(100):
        	x, y = np.where(self.H_D == i)
        	self.pos_D[i,0]=x
        	self.pos_D[i,1]=y
        '''
        ...
        '''
        order_P = 32
        self.H_P = hilbert_curve(order_P)

        self.pos_P = np.zeros((1000,2)).astype(int)
        for i in range(1000):
        	x, y = np.where(self.H_P == i)
        	self.pos_P[i,0]=x
        	self.pos_P[i,1]=y
        '''
        ...
        '''
        self.pos_P = np.zeros((128*128,2)).astype(int)
        for i in range(128*128):
        	x, y = np.where(self.H_P == i)
        	self.pos_P[i,0]=x
        	self.pos_P[i,1]=y
        '''
        self.feature_p = feature_p

        if phase == 'train':
            self.scl_feature_p = np.load('/home/yan/code/DTI/DTI_Best_Version_1018/train_drug_1.npy')
            self.scl_feature_d = np.load('/home/yan/code/DTI/DTI_Best_Version_1018/train_protein_1.npy')
        elif phase == 'valid':
            self.scl_feature_p = np.load('/home/yan/code/DTI/DTI_Best_Version_1018/valid_drug_1.npy')
            self.scl_feature_d = np.load('/home/yan/code/DTI/DTI_Best_Version_1018/valid_protein_1.npy')
        elif phase == 'test':
            self.scl_feature_p = np.load('/home/yan/code/DTI/DTI_Best_Version_1018/test_drug_1.npy')
            self.scl_feature_d = np.load('/home/yan/code/DTI/DTI_Best_Version_1018/test_protein_1.npy')
    ...
    def __getitem__(self, index):
        ...
        while(1):
            try:
                ...
                rdkit_mol = AllChem.MolFromSmiles(d)
                mgf = torch.tensor(getmorganfingerprint(rdkit_mol))
                maccs = torch.tensor(getmaccsfingerprint(rdkit_mol))
                esm_feature = self.feature_p[index]

                v_d, l_drug = trans_drug(d)
                v_p, l_protein = trans_protein(p)
        
                if self.config['drug_encoding'] == 'CNN' or self.config['drug_encoding'] == 'Inception' or self.config['drug_encoding'] == 'CNN1D':
                    v_d = label_smiles(v_d)
                if self.config['drug_encoding']=='CNN' or self.config['drug_encoding'] == 'Inception':
                    v_d = drug_to_hilbert(v_d,self.H_D)

                if self.config['target_encoding'] == 'CNN' or self.config['target_encoding'] == 'Inception' or self.config['target_encoding'] == 'CNN1D':
                    v_p = label_sequence(v_p) 

                if self.config['target_encoding']=='CNN' or self.config['target_encoding'] == 'Inception':
                    v_p = protein_to_hilbert(v_p,self.H_P)
                y = self.labels[index]
                return v_d, v_p, y, l_drug, l_protein, torch.cat((mgf,maccs),0), esm_feature#torch.cat((esm_feature, scl_feature_p),0)
            except:
                index = 0
...
class data_process_loader(data.Dataset):
	...
	def __getitem__(self, index):
		...
		if self.config['drug_encoding'] == 'CNN' or self.config['drug_encoding'] == 'CNN_RNN' or self.config['drug_encoding'] == 'CNN1D':
			v_d = drug_2_embed(v_d)
		if self.config['drug_encoding']=='CNN':
			v_d = drug_to_hilbert(v_d,self.H_D)
		v_p = self.df.iloc[index]['target_encoding']
		if self.config['target_encoding'] == 'CNN' or self.config['target_encoding'] == 'CNN_RNN' or self.config['target_encoding'] == 'CNN1D':
			v_p = protein_2_embed(v_p)

		if self.config['target_encoding']=='CNN':
			v_p = protein_to_hilbert(v_p,self.H_P)
		y = self.labels[index]
		return v_d, v_p, y
...
enc_protein_test = OneHotEncoder().fit(np.array(smiles_char).reshape(-1, 1))
...
def trans_protein_test(x):
	temp = list(Chem.MolToSmiles(Chem.MolFromFASTA(x)))
	#temp = list(temp.upper())
	#temp = [i if i in smiles_char else '?' for i in temp]
	if len(temp) < 128*128:
		temp = temp + ['?'] * (128*128-len(temp))
	else:
		temp = temp [:128*128]
	return temp


def protein_2_embed_test(x):
    return enc_protein_test.transform(np.array(x).reshape(-1,1)).toarray().T
