...
from sklearn.metrics import f1_score,roc_auc_score, average_precision_score, f1_score, roc_curve, confusion_matrix, precision_score, recall_score, auc, log_loss
...
class DTI:
	...
	def test_(self, data_generator, model, repurposing_mode = False, test = False):
		...
		if self.binary:
			...
			return auc_k, auprc, f1_score(y_label, outputs), sensitivity1, specificity1, accuracy1, pre, log_loss(y_label,y_pred), y_pred, y_label
	...
	def train(self, train, val, test, verbose = True,datanum=0,setnum=0):
		...
		for epo in range(train_epoch):
			...
			with torch.set_grad_enabled(False):
				...
				if self.binary:  
					...
					auc, auprc, f1, sensitivity, specificity, acc, precision, loss, logits, _ = self.test_(valid_loader, self.model)	
					...
					if epo%20==0:
						...
						if test is not None:
							...
							if self.binary:
								auc, auprc, f1, sensitivity, specificity, acc, precision, loss, logits, _ = self.test_(test_loader, model_max, test=True)
		...
		with torch.set_grad_enabled(False):
			if verbose:
				print('--- Go for Testing ---')
			if self.binary:
				auc, auprc, f1, sensitivity, specificity, acc, precision, loss, test_pred, test_label = self.test_(test_loader, model_max, test = True)
				test_table = PrettyTable(["AUROC", "AUPRC", "F1","Sensitivity","Specificity","Accuracy","Precision"])
				test_table.add_row(list(map(float2str, [auc, auprc, f1, sensitivity, specificity, acc, precision])))
				test_table.add_row(['1','1','1','1','1','1',str(best_epo)])
				if verbose:
					print('Test at Epoch: '+ str(epo + 1) + ', AUROC: ' + str(auc)[:7] + \
						' , AUPRC: ' + str(auprc)[:7] +' , sensitivity: ' +str(sensitivity)+' , specificity: ' +str(specificity)+ ',acc:'+str(acc)+ ' , F1: '+str(f1)[:7] + ' , Precision: ' + str(precision)[:7] + ' , Cross-entropy Loss: ' + \
						str(loss)[:7])	
		...
		return auc, auprc, f1, sensitivity, specificity, acc, precision
